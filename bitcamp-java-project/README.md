...
## src14 - DAO 역할 클래스 식별
컨트롤러 클래스에서 데이터를 보관처리하는 코드 부분을 별도의 클래스로 분리 

-학습 목표
    -기존 클래스의 역할 중에 일부를 분리하는 방법을 익힌다.
    -DAO 역할에 대해 이해한다.
-작업내용
    -컨트롤러 클래스에서 데이터르 보관하고 코드 부분을 별도의 클래스로 분리한다.
## src13 - 날짜 값은 java.sql.Date 클래스를 작영(리펙토링)
-학습목표
- java.sql.Date 클래스의 사용법을 익힌다.
-작업내용
    -팀,회원,게시물 정보 입력/ 변경변경 시 날짜 값을 Date 객체에 저장한다. 
...
## src11 - 인스턴스 변수와 인스턴스 메서드 활용(리펙토링)
 - 학습목표
 -인스턴스 변수의 사용법을 익힌다.
 -인스턴스 메서드의 사용법을 익힌다.
 -작업내용
 -스태틱 변수와 스태택 메서드로 마든 기준 코드를 인스턴스 변수와 인스턴스 메서드로 변경한다.
 BoardControoler.java 변경
 TeamControoler.java 변경
 MemberControoler.java 변경
 App.java변경
