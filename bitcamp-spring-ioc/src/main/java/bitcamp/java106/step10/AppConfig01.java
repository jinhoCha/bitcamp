...
public class AppConfig01 {
    ...
    // 객체의 이름을 지정하지 않으면 메서드 이름을 객체 이름으로 사용한다.
       ...
       @Bean("car1")
        public Car getCar() {
        Car c = new Car(null); // 의존객체를 요구하는대  null값을 넣음
        c.setMaker("비트자동차");
        c.setModel("티코");
        c.setCc(890);
        c.setAuto(true);
       ...
       @Bean("car2") // 이렇게 이 메서드가 리턴한 객체를 보관할 때 사용할 이름을 지정할 수 있다.
       public Car getCar2() {
       Car c = new Car(null); // 의존객체를 요구하는대  null값을 넣음
       c.setMaker("비트자동차");
       c.setModel("티코");
