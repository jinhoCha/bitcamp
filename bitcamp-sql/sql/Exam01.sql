...
> create table test1(
  c1 int, 
  c2 float, 
  c3 numeric(6,2), /* 6자리중 소수점2자리 4자리정수 2자리 소수 */
  c4 numeric
  );
  
> insert into test1(c1) values(100);
> insert into test1(c1) values(3.14); /* 소수점 짤림 */
> insert into test1(c2) values(100.98); /* 소수점 이하 반올림하고 짜름 */
...
> insert into test1(c1) values('가나다라마바'); /* 한글자 더많아서 오류 */
...
즉 c1 ='abc'에서는 데이터를 찾지 못하고, c1='abc  '이여야만 데이터를 찾는 경우가 있다.
mysql은 고정크기 컬럼이더라도 빈자리를 무시하고 데이터를 찾는다.
...
> create table test1(
  ...
  eng int,
  math int,
  constraint test_pk primary key(name, age)
  );
...
> insert into test1(name, age, kor, eng, math) values('ab', 11, 88, 88, 88);
    insert into test1(name, age, kor, eng, math) values('aa', 10, 88, 88, 88); // 에러
...
- 여러 개의 컬럼을 묶어서 PK로 사용하면 데이터를 다루기가 불편하다.
 즉 데이터를 찾을때 마다 name과 age 값을 지정해야 한다.
...
- 입력 테스트:
 >insert into test1(no,name,age,kor,eng,math) values(1,'a',10,90,90,90);
 >insert into test1(no,name,age,kor,eng,math) values(2,'a',11,91,91,91);
 >insert into test1(no,name,age,kor,eng,math) values(3,'b',11,81,81,81);,,,,,,,,,,,,ㅜㅡㅏㅣㅣ ㅓㅣㅜㅑㅐㅜㅕㅑㅐㅜㅡㅑㅐ[ㅜㅡㅕㅑㅐㅜ[ㅠㅕㅓㅐ 
 ㅕㅐ
 ㅕ
 >insert into test1(no,name,age,kor,eng,math) values(4,'c',20,81,81,81);
...
alter table test1
  ...
  add constraint test_1uk unique (name, age),
...
- 입력 테스트
```
insert into test11(no,name,age,kor,eng,math,sum,aver)
...
```
alter table test1
  modify column no int not null auto_increment; /* 아직 no가 pk가 아니기 떄문에 오류*/
...
- 입력 테스트
```
insert into test1(name) values('aaa'); /* 번호를 지정하지않고 이름만준다.*/
