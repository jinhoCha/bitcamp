...
insert into test1(name,fax,tel,no,pstno,addr) 
    values('bbb','222','111',null,'10101','seoul'); /*명시한 순서대로 값을 줘야한다.*/
```
...
```
### 다른테이블의 결과를 넣을수가있따 test1 -> test2로 넣는방법 서울인사람만
...
##autocommit 
mysql은 autocommit의 기본값이 true이다. 따라서 명령창에서 SQL을 실행하면 바로 실제 테이블에
적용된다. 수동으로 처리하고 싶다면 autocommit을 false로 설정하라!

set autocommit=false;

insert/update/delete를 수행한후 승인을해야만 실제 테이블에 적용된다.
```
> commit

```
마지막 commit상태로 되돌리고 싶다면,
```
> rollback
```
...
rollback /*지금 까지 작업한 inser,udpdate/ delete은 취소된다.
```
