...
import java.util.Scanner;
     //상수를 좀더 조직적으로 관리하는 방법
                // => enum을 사용하여 상수를 정의한다.

 enum Level {
         GUTEST2, MEMBER2, ADMIN2
...
public class Exam02_4 {
    ...
    public static void main (String[] args){
     ...
     //상수를 사용하지 않았을때
     // => 값의 의미를 주석으로 달아야만 다른 개발자가 이해할수 있다.
        switch(level)  {
            case 0:// 손님
            ...
            final int GUTEST = 0, MEMBER =1, ADMIN = 2;
            ...
            // => 값을 직접 지정할 수도 있따. (나중에 따로 enum문법 설명)
            // enum을 사용하는 주된 이유는 ?
            // => 100, 200, "admin" 과 같이 값을 직접 지정할 필요가 없다.
            // => enum 변수에는 그 타입에 정의된 값만 저장할 수 있도록 제어할 수 있다.
            Level userLevel = Level.MEMBER2; // 지금은 맴버로만 지정해서 글작성가능합니다가 찍힌다.
            
            //다음과 같이 switch나 case 값으로 enum 타입의 값이 올 수 도 있다.
            switch(userLevel)  {
                case GUTEST2:// 손님
                    System.out.println("조회만 가능합니다.");
                    break;
...
//if와 vs switch 
// 복잡한 조건으로 프로그램 흐름을 제어하는 상황이라면 if문을 사용하라!!!
