...
public class Score3 {
  ...
  //다음 연산자는 계산을 수행 할때마다 인스턴스의 주소를 파라미터로 받아야한다.
   ...
   */
   ...
   // 메서드를 호출할 때 (연산자를 사용할 때) , 메서드 앞에 인스턴스의 주소를 적는다.
   //=> 이 인스턴스에 주소는 내장된 this라는 변수에 자동 복사된다.
...
// 클래스 메서드
//=> static이 붙는 메서드
//=> 특정 인스턴스가 아닌 모든 인스턴스에 대해 작업을 수행할 떄 사용하는 메서드이다.
// 사용법 = > 클래스명.메서드명();
//     = > 레퍼런스.메서드명();
...
// => 인스턴스 메서드
// => static이 붙지않는 메서드
// => 특정 인스턴스에 대해 작업을 수행할떄 사용하는 메서드이다.
// = > 사용법
//  => 레퍼런스.메서드명();