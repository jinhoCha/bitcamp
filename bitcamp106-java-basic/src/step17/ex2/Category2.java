...
public class Category2 {
    ...
    // 여러 상수를 그룹별로 묶어 관리한다.
    // => 그러면 차라리 각각의 클래스를 맴버 클래스로 별도의 파일로 분맇나느 것이 낫지 않을까?
    //=> 다음 클래스를 보면 상수를 묶어서 관리하는 용도로 만들었다.
    
    //=> 클래스의 크기도 작다.
    //=-> 이런 클래스를 별도의 파일로 분리하면 여러 개의 파일이 생겨서 오히려 관리하기 번거롭다.
    // 즉 자잘한 클래스들이 여러 파일에 분산되기 떄문에 관리하기 번거로워 진다.
    // 이렇게 중첩 클래스로 만들면 사용할 때 계층적으로 작성하기 때문에 이해하기가 쉬워진다
    // int value = category.computer.CPU;
    // => 바깥 클래스를 패키지처럼 생각할 수 있어 이해하기 관리하고 편하다.
